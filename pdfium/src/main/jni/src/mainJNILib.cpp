...
JNI_FUNC(jboolean, PdfiumCore, nativeGetMixedLooseCharPos)(JNI_ARGS, jlong pagePtr, jint offsetY,
    ...
    FS_RECTF res = {0};
//TODO
//    FPDF_BOOL result = FPDFText_GetLooseCharBox((FPDF_TEXTPAGE) textPtr, idx, &res);
//    if (!result) {
//        return false;
//    }
...
JNI_FUNC(void, PdfiumCore, nativeGetCharPos)(JNI_ARGS, jlong pagePtr, jint offsetY, jint offsetX,
    ...
    if (loose) {
        FS_RECTF res = {0};
        //TODO
        //FPDFText_GetLooseCharBox((FPDF_TEXTPAGE) textPtr, idx, &res);
        left = res.left;
        top = res.top;
        right = res.right;
        bottom = res.bottom;
...
JNI_FUNC(jboolean, PdfiumCore, nativeGetRect)(JNI_ARGS, jlong pagePtr, jint offsetY, jint offsetX,
    ...
    FPDFText_GetRect((FPDF_TEXTPAGE) textPtr, idx, &left, &top, &right, &bottom);
    int deviceX, deviceY;
...
//
    ...
    for (int i = 0; i < rectCount; i++) {//"RectF(373.0, 405.0, 556.0, 434.0)"
        FPDFText_GetRect((FPDF_TEXTPAGE) textPtr, i, &left, &top, &right, &bottom);
